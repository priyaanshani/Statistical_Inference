...
### Lambda = 0.2, 40 samples, 2500 trials
```{r echo=FALSE}  
library(ggplot2) 
```
```{r}
set.seed(314159)  
lambda <- 0.2;    number.samples <- 40;     nosim <- 2500
...
```{r histogramOfMeans, fig.width=6, fig.height=4, fig.cap="Histogram of Sample Means", warning=FALSE} 
...
```
**The distribution of sample means comes close to a normal distribution(red)**


## Testing Confidence Levels

### Lambda 0.1 to 100.0, 40 samples, 2500 trials
...
**The value of Lambda does not seem to matter.**
